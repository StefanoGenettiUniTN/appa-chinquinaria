"""
Script to create curated ARPAV PM10 datasets from the raw merged CSV.
...
7. Saves both a clean dataset (no metadata columns) and a version with
   interpolation metadata for downstream QA/visualization
"""
...
def interpolate_station_data(station_df, max_gap_hours=4):
    ...
    # Merge with actual data (include interpolation metadata if already present)
    ...
    merge_columns = ['datetime', 'pm10', 'station_code', 'station_name']
    if 'interpolation_method' in station_df_sorted.columns:
        merge_columns.append('interpolation_method')
    
    complete_df = complete_df.merge(
        station_df_sorted[merge_columns],
        on='datetime',
        how='left'
    )
    ...
    # Initialize interpolation method tracking
    if 'interpolation_method' not in complete_df.columns:
        complete_df['interpolation_method'] = 'missing'
    else:
        complete_df['interpolation_method'] = complete_df['interpolation_method'].fillna('missing')
    
    original_pm10 = complete_df['pm10'].copy()
    existing_actual_mask = original_pm10.notna() & (complete_df['interpolation_method'] == 'missing')
    complete_df.loc[existing_actual_mask, 'interpolation_method'] = 'actual'
    ...
    # Update interpolation metadata
    newly_filled_mask = original_pm10.isna() & complete_df['pm10'].notna()
    complete_df.loc[newly_filled_mask, 'interpolation_method'] = 'linear'
...
def create_curated_arpav_dataset(
    input_file,
    output_file=None,
    output_file_with_metadata=None,
    max_gap_hours=4
):
    """
    ...
    """
    print("=" * 80)
    print("ARPAV PM10 Dataset Curation")
    print("=" * 80)
    ...
    # Reorder columns (keep interpolation metadata)
    result_df = result_df[['datetime', 'station_code', 'station_name', 'pm10', 'interpolation_method']]
    ...
    # ============================================================================
    # Step 9: Save curated datasets (with + without interpolation metadata)
    # ============================================================================
    ...
    if output_file_with_metadata is None:
        output_file_with_metadata = output_file.parent / f"{output_file.stem}_with_interp_metadata.csv"
    output_file_with_metadata = Path(output_file_with_metadata)
    
    print(f"\n9. Saving curated datasets to:")
    output_file.parent.mkdir(parents=True, exist_ok=True)
    output_file_with_metadata.parent.mkdir(parents=True, exist_ok=True)
    
    # Save metadata-rich version
    metadata_columns = ['datetime', 'station_code', 'station_name', 'pm10', 'interpolation_method']
    result_df[metadata_columns].to_csv(output_file_with_metadata, index=False)
    print(f"   • {output_file_with_metadata} (with interpolation metadata)")
    
    # Save clean version without metadata column
    clean_df = result_df[['datetime', 'station_code', 'station_name', 'pm10']]
    clean_df.to_csv(output_file, index=False)
    print(f"   • {output_file} (clean)")
    print(f"   ✓ Saved {len(result_df):,} rows per version")
    ...
    actual_measurements = (result_df['interpolation_method'] == 'actual').sum()
    linear_measurements = (result_df['interpolation_method'] == 'linear').sum()
    remaining_missing = (result_df['interpolation_method'] == 'missing').sum()
    print(f"   Measurements from sensors: {actual_measurements:,}")
    print(f"   Linear interpolations: {linear_measurements:,}")
    ...
    for station_code in sorted(result_df['station_code'].unique()):
        ...
        linear_count = (station_data['interpolation_method'] == 'linear').sum()
        print(f"     {station_code} ({station_name}): {count:,} rows ({linear_count:,} linear fills)")
...
if __name__ == "__main__":
    ...
    output_file_with_metadata = project_root / "data" / "arpav" / "PM10" / "merged_pm10_hourly_curated_with_interp_metadata.csv"
    
    # Run curation
    create_curated_arpav_dataset(
        input_file,
        output_file=output_file,
        output_file_with_metadata=output_file_with_metadata,
        max_gap_hours=4
    )
